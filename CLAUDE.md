...
- `src/lib/claude.ts` — all LLM prompts and inference logic
...
| LLM | OpenRouter `gpt-oss-20b:free` (free tier); provider-agnostic wrapper in `claude.ts` |
...
1. **All LLM calls go through `callLLM()` in `lib/claude.ts`** — never call OpenRouter directly from a route
...
- [ ] Profile extraction prompt (`extractProfile()` in `claude.ts`)
...
3. All LLM calls go through callLLM() in lib/claude.ts
