...
export default function AdminDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      const [pendingRes] = await Promise.all([
        fetch('/api/kb/approve?status=pending_admin')
      ])
  ...
  const handleApprove = async (id: string) => {
    ...
    await fetch('/api/kb/approve', {
      ...
      body: JSON.stringify({ action: 'admin_approve', kb_entry_id: id, approved_by: 'admin' })
    })
    setSelected(null)
    await loadDashboard()
  ...
  const handleReject = async (id: string) => {
    ...
    await fetch('/api/kb/approve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'admin_reject', kb_entry_id: id })
    })
    setSelected(null)
    await loadDashboard()
    setActionLoading(false)
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/40 text-sm mt-0.5">
                      {entry.topic} · SME: {(entry as any).sme?.name || entry.sme_id}
                    </p>
                    <p className="text-white/60 text-sm mt-2 line-clamp-2 leading-relaxed">{entry.content}</p>
                  </div>
                  <div className="flex gap-2 ml-4 shrink-0">
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); handleReject(entry.id) }}
                      disabled={actionLoading}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-6 z-50">
          <div className="bg-[#111] border border-white/15 rounded-2xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-white text-lg font-medium">{selected.title}</h3>
              <button onClick={() => setSelected(null)} className="text-white/40 hover:text-white text-xl">×</button>
            </div>

            <div className="space-y-4 mb-6">
              <div>
                <label className="text-white/40 text-xs uppercase tracking-widest block mb-1">Topic</label>
                <p className="text-white/80 text-sm">{selected.topic}</p>
              </div>
              <div>
                <label className="text-white/40 text-xs uppercase tracking-widest block mb-1">Content</label>
                <p className="text-white/80 text-sm leading-relaxed">{selected.content}</p>
              </div>
              <div>
                <label className="text-white/40 text-xs uppercase tracking-widest block mb-1">Keywords</label>
              ...
              <div>
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${selected.visibility === 'user_visible' ? 'bg-emerald-400/10 text-emerald-400' : 'bg-white/5 text-white/40'}`}>
                  {selected.visibility}
                </span>
              </div>
            </div>
