...
export default function SMEOnboarding({ smeProfile }: { smeProfile: SMEProfile }) {
  ...
  const pendingEntries = entries.filter(e => e.status === 'pending_review')
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm">{smeProfile.title} · {smeProfile.email} · {smeProfile.domain}</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Awaiting your review', value: draftEntries.length, color: 'text-yellow-400' },
          { label: 'Pending admin', value: pendingEntries.length, color: 'text-blue-400' },
          { label: 'Live in KB', value: approvedEntries.length, color: 'text-emerald-400' },
        ].map(s => (
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(smeProfile.topics || []).length === 0 && (
            <span className="text-white/30 text-sm">No topics yet</span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <h3 className="text-white font-medium mb-1">Add a New Topic</h3>
          <p className="text-white/40 text-sm">Start an interview to capture more expertise</p>
        </button>
        <button
          onClick={loadEntries}
          disabled={loadingEntries}
          className="group border border-white/10 hover:border-[#e20074]/60 rounded-xl p-6 text-left transition-all hover:bg-white/5 disabled:opacity-50"
        >
          <div className="text-2xl mb-3">🔄</div>
          <h3 className="text-white font-medium mb-1">Refresh Entries</h3>
          <p className="text-white/40 text-sm">{loadingEntries ? 'Loading...' : 'Reload your entry list'}</p>
        </button>
      </div>

      {/* Entries awaiting SME review */}
      {draftEntries.length > 0 && (
        <div className="mb-8">
          <h3 className="text-white/60 text-xs uppercase tracking-widest mb-4">
            Awaiting Your Review ({draftEntries.length})
          </h3>
          <div className="space-y-3">
            {draftEntries.map(entry => (
              <div key={entry.entry_id} className="border border-yellow-400/30 bg-yellow-400/5 rounded-xl p-4 flex items-center justify-between">
                <div>
                ...
                <button
                  onClick={() => { setSelectedEntry(entry); setView('review_entry') }}
                  className="ml-4 shrink-0 text-[#e20074] text-xs hover:underline"
                >
                  Review →
                </button>
              </div>
      ...
      {entries.length > 0 && (
        <div>
          <h3 className="text-white/60 text-xs uppercase tracking-widest mb-4">All Your Entries</h3>
...
function InterviewFlow({ smeProfile, topicById, onComplete, onBack }: {
  ...
  const sendMessage = async () => {
    ...
    const userMsg: InterviewMessage = { role: 'sme', content: input, timestamp: new Date().toISOString() }
    ...
    try {
      ...
      if (data.status === 'completed') setTimeout(() => synthesize(), 1000)
  ...
  if (phase === 'setup') {
    return (
      <div className="max-w-2xl mx-auto p-8">
        ...
        <p className="text-white/40 text-sm mb-8">Thoth will guide you through a structured interview to capture your expertise on a new topic.</p>
    ...
    return (
      <div className="max-w-3xl mx-auto p-8">
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 mb-6">
          <p className="text-emerald-400 text-sm">✓ {kbEntries.length} {kbEntries.length === 1 ? 'entry' : 'entries'} created as drafts. Go to your dashboard to review and approve them.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4 max-w-3xl mx-auto w-full">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'sme' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${msg.role === 'sme' ? 'bg-[#e20074] text-white rounded-tr-sm' : 'bg-white/8 text-white/85 border border-white/10 rounded-tl-sm'}`}>
              {msg.content}
            </div>
          </div>
        ))}
...
function EntryReview({ entry, smeId, topicById, onComplete, onBack }: {
  ...
  const handleApprove = async () => {
    ...
    await fetch('/api/kb/approve', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'sme_approve', kb_entry_id: entry.entry_id, sme_id: smeId, edits: { synthesized_answer: synthesizedAnswer } })
    })
    setStatus('approved')
    setLoading(false)
  }
  ...
  const handleReject = async () => {
    ...
    await fetch('/api/kb/approve', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'sme_reject', kb_entry_id: entry.entry_id, edits: { synthesized_answer: synthesizedAnswer } })
    })
    setStatus('rejected')
    setLoading(false)
  }
  ...
  if (status !== 'idle') {
    return (
      <div className="max-w-2xl mx-auto p-8 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-white text-lg font-light mb-2">{status === 'approved' ? 'Submitted for admin review!' : 'Returned to draft'}</p>
        <button onClick={onComplete} className="mt-6 text-[#e20074] text-sm hover:underline">Back to Dashboard</button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      <button onClick={onBack} className="text-white/40 hover:text-white text-sm mb-8">← Back</button>
      <h2 className="text-white text-2xl font-light mb-6">Review Knowledge Entry</h2>
      <div className="space-y-4 mb-6">
        <div>
          <label className="text-white/60 text-xs uppercase tracking-widest block mb-2">Topic(s)</label>
        ...
        <div>
          ...
          <p className="text-white/70 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm">{entry.question_framing}</p>
        </div>
        <div>
          <label className="text-white/60 text-xs uppercase tracking-widest block mb-2">Answer (editable)</label>
          <textarea value={synthesizedAnswer} onChange={e => setSynthesizedAnswer(e.target.value)}
            rows={8}
        ...
        <div>
          ...
          <span className={`text-xs px-2 py-1 rounded-full ${entry.exposable_to_users ? 'bg-emerald-400/10 text-emerald-400' : 'bg-white/5 text-white/40'}`}>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${config[status] || 'text-white/40 bg-white/5'}`}>
      {status.replace('_', ' ')}
    </span>
  )
}
