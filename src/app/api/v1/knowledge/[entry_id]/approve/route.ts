...
export async function POST(req: NextRequest, { params }: { params: Promise<{ entry_id: string }> }) {
  ...
  try {
    ...
    if (entry.status !== 'draft') {
      return NextResponse.json({ error: 'Invalid state transition: entry must be in draft status', code: 'INVALID_TRANSITION' }, { status: 409 })
    }

    const now = new Date().toISOString()
    await kbV1Api.smeApprove(entry_id, entry.sme_id)

    return NextResponse.json({ entry_id, status: 'sme_approved', approved_at: now })
  } catch (err: any) {
